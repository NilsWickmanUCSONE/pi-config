...
function withEmphasizedIcon(icon: string, text: string): string {
  // Terminals cannot scale individual glyphs, but bolding the icon makes it
  // read larger/more prominent while keeping the cell compact.
  return icon ? `\x1b[1m${icon}\x1b[22m ${text}` : text;
}
...
const modelSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    let content = withEmphasizedIcon(icons.model, modelName);
...
const gitSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    // Keep this cell compact: icon + branch only. Dirty/clean still affects color,
    // but staged/unstaged/untracked counts are intentionally hidden.
    return { content: color(ctx, branchColor, withIcon(icons.branch || icons.git, branch)), visible: true };
  },
};
...
const thinkingSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    if (level === "high" || level === "xhigh") {
      // Keep the Nerd Font brain glyph outside the per-character rainbow
      // gradient. Some terminals/font fallback paths fail to render private-use
      // glyphs when every glyph receives a separate truecolor escape sequence.
      return { content: withIcon(icons.thinking, rainbow(label)), visible: true };
    }
