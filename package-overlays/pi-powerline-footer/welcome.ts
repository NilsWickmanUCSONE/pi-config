...
function getCodexEmail(): string | undefined {
  try {
    ...
    const accessToken = auth["openai-codex"]?.access;
...
function buildLeftColumn(data: WelcomeData, colWidth: number): string[] {
  ...
  return [
    ...
    ...(codexEmail ? [centerText(dim(codexEmail), colWidth)] : []),
  ];
}
...
function buildRightColumn(data: WelcomeData, colWidth: number): string[] {
  ...
  // Loaded resource lines. Prefer compact names so the overlay replaces Pi's
  // separate startup rows instead of merely duplicating their counts.
  const countLines: string[] = [];
  const { contextFiles, extensions, skills, promptTemplates } = data.loadedCounts;
  const details = data.loadedDetails;
  const itemPrefix = dim("- ");
  const compactList = (items: string[], empty: string) => items.length > 0 ? items.join(", ") : empty;

  if (details) {
    if (details.contexts.length > 0) countLines.push(` ${itemPrefix}${fgOnly("gitClean", `${details.contexts.length}`)} context: ${dim(compactList(details.contexts, ""))}`);
    if (details.skills.length > 0) countLines.push(` ${itemPrefix}${fgOnly("gitClean", `${details.skills.length}`)} skills: ${dim(compactList(details.skills, ""))}`);
    if (details.extensions.length > 0) countLines.push(` ${itemPrefix}${fgOnly("gitClean", `${details.extensions.length}`)} extensions: ${dim(compactList(details.extensions, ""))}`);
    if (details.promptTemplates.length > 0) countLines.push(` ${itemPrefix}${fgOnly("gitClean", `${details.promptTemplates.length}`)} prompts: ${dim(compactList(details.promptTemplates, ""))}`);
    if (details.themes.length > 0) countLines.push(` ${itemPrefix}${fgOnly("gitClean", `${details.themes.length}`)} themes: ${dim(compactList(details.themes, ""))}`);
  }

  if (countLines.length === 0 && (contextFiles > 0 || extensions > 0 || skills > 0 || promptTemplates > 0)) {
    if (contextFiles > 0) {
  ...
  return [
    ...
    ` ${dim("/")} commands  ${dim("!")} bash  ${dim("Ctrl+O")} more`,
    ` ${dim("Esc")} interrupt  ${dim("Ctrl+C/Ctrl+D")} clear/exit`,
...
export class WelcomeComponent implements Component {
  ...
  constructor(
    ...
    this.data = { modelName, providerName, recentSessions, loadedCounts, loadedDetails };
  }
...
export function discoverLoadedDetails(): LoadedDetails {
  const homeDir = process.env.HOME || process.env.USERPROFILE || osHomedir();
  const cwd = process.cwd();
  const details: LoadedDetails = { contexts: [], extensions: [], skills: [], promptTemplates: [], themes: [] };
  const seen = {
    contexts: new Set<string>(),
    extensions: new Set<string>(),
    skills: new Set<string>(),
    promptTemplates: new Set<string>(),
    themes: new Set<string>(),
  };
  const push = (key: keyof LoadedDetails, value: string | undefined) => {
    const label = value?.trim();
    if (!label || seen[key].has(label)) return;
    seen[key].add(label);
    details[key].push(label);
  };
  const shortPath = (path: string) => path.replace(homeDir, "~").replace(cwd, ".");
  ...
  for (const path of [
    ...
    if (existsSync(path)) push("contexts", shortPath(path));
  }
    ...
    try {
      for (const entry of readdirSync(dir)) {
        ...
        try {
          ...
          logDiscoveryError(`Failed to inspect skill detail ${entryPath}`, error);
        }
      }
    } catch (error) {
      logDiscoveryError(`Failed to scan skill details dir ${dir}`, error);
    }
  };

  const scanPromptTemplates = (dir: string) => {
    if (!existsSync(dir)) return;
    try {
      for (const entry of readdirSync(dir)) {
        const entryPath = join(dir, entry);
        try {
          const stats = statSync(entryPath);
          if (stats.isDirectory()) scanPromptTemplates(entryPath);
          else if (entry.endsWith(".md")) push("promptTemplates", `/${basename(entry, ".md")}`);
        } catch (error) {
          logDiscoveryError(`Failed to inspect prompt detail ${entryPath}`, error);
        }
      }
    } catch (error) {
      logDiscoveryError(`Failed to scan prompt details dir ${dir}`, error);
    }
  };

  const scanExtensions = (dir: string) => {
    if (!existsSync(dir)) return;
    try {
      for (const entry of readdirSync(dir)) {
        const entryPath = join(dir, entry);
        try {
          ...
          if (stats.isDirectory()) {
            ...
            push("extensions", basename(entry, entry.endsWith(".ts") ? ".ts" : ".js"));
          }
        } catch (error) {
          logDiscoveryError(`Failed to inspect extension detail ${entryPath}`, error);
        }
      }
    } catch (error) {
      logDiscoveryError(`Failed to scan extension details dir ${dir}`, error);
    }
  };

  scanSkills(join(homeDir, ".pi", "agent", "skills"));
  scanSkills(join(homeDir, ".agents", "skills"));
  scanSkills(join(cwd, ".pi", "skills"));
  scanSkills(join(cwd, ".agents", "skills"));
  scanExtensions(join(homeDir, ".pi", "agent", "extensions"));
  scanExtensions(join(cwd, ".pi", "extensions"));
  scanPromptTemplates(join(homeDir, ".pi", "agent", "prompts"));
  scanPromptTemplates(join(homeDir, ".pi", "agent", "commands"));
  scanPromptTemplates(join(cwd, ".pi", "prompts"));
  scanPromptTemplates(join(cwd, ".pi", "commands"));

  for (const settingsPath of [join(homeDir, ".pi", "agent", "settings.json"), join(cwd, ".pi", "settings.json")]) {
    if (!existsSync(settingsPath)) continue;
    try {
      const settings = JSON.parse(readFileSync(settingsPath, "utf-8"));
      if (typeof settings?.theme === "string") push("themes", settings.theme);
      const packages = Array.isArray(settings?.packages) ? settings.packages : [];
      for (const pkg of packages) {
        const source = typeof pkg === "string" ? pkg : typeof pkg?.source === "string" ? pkg.source : undefined;
        if (!source?.startsWith("npm:")) continue;
        const body = source.slice(4);
        const versionIndex = body.lastIndexOf("@");
        const packageName = versionIndex > 0 ? body.slice(0, versionIndex) : body;
        const packageRoot = join(homeDir, ".pi", "agent", "npm", "node_modules", packageName);
        const manifestPath = join(packageRoot, "package.json");
        push("extensions", packageName === "pi-powerline-footer" ? "pi-powerline-footer" : packageName);
        if (!existsSync(manifestPath)) continue;
        const manifest = JSON.parse(readFileSync(manifestPath, "utf-8"));
        const pi = manifest?.pi ?? {};
        for (const skillPath of Array.isArray(pi.skills) ? pi.skills : []) {
          if (typeof skillPath === "string") scanSkills(join(packageRoot, skillPath));
        }
        for (const extPath of Array.isArray(pi.extensions) ? pi.extensions : []) {
          if (typeof extPath === "string") push("extensions", extPath.replace(/^\.\//, "").replace(/\/index\.(ts|js)$/, ""));
        }
        for (const themePath of Array.isArray(pi.themes) ? pi.themes : []) {
          if (typeof themePath === "string") push("themes", basename(themePath).replace(/\.(json|ts|js)$/, ""));
        }
      }
    } catch (error) {
      logDiscoveryError(`Failed to read loaded details from ${settingsPath}`, error);
    }
  }

  for (const key of Object.keys(details) as Array<keyof LoadedDetails>) {
    details[key].sort((a, b) => a.localeCompare(b));
  }
  return details;
}
