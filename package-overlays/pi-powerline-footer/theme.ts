...
export function rainbow(text: string): string {
  ...
  for (let i = 0; i < text.length;) {
    ...
    // Preserve existing ANSI escape sequences (for example bold icon
    // markers) instead of coloring their bytes as printable text. Coloring
    // inside the sequence corrupts it and leaks strings like "[1m" / "[22m"
    // into the footer for high/xhigh thinking levels.
    if (char === "\x1b") {
