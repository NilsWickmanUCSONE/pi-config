...
export class TerminalSplitCompositor {
  ...
  private restoreTerminalState(options: DisposeOptions = {}): void {
    ...
    const shouldExitAlternateScreen = options.exitAlternateScreen === true || options.resetExtendedKeyboardModes === true;
    const restoreMainScreenMode = shouldExitAlternateScreen
    ...
    this.originalWrite(
      ...
      + enableAlternateScrollMode()
      // Local patch: during Pi session replacement (/new, /reload), do not leave
      // the alternate screen. Leaving it briefly exposes the parent terminal UI and
      // looks like Pi got thrown out of the TUI. Full shutdown still exits via
      // resetExtendedKeyboardModes/session_shutdown.
      + (shouldExitAlternateScreen ? exitAlternateScreen() : "")
